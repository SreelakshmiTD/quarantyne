...
**Note on the dashboard demo data:** the 1,981 events and 752 violations visible in the dashboard screenshots were generated by `seed_demo.py`, which inserts directly into the audit tables to produce realistic-looking demo data. This is separate from the load test — it does not go through Kafka or the consumer pipeline and does not reflect pipeline throughput. The figures above (2,336.9 msg/s, 0.3ms max latency) are from `load_test.py` running against the live consumer pipeline end-to-end.
